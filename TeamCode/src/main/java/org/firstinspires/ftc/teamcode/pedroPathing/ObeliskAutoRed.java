...
public class ObeliskAutoRed extends OpMode {
    ...
    private final Pose startPose = new Pose(120, 128, Math.toRadians(270));// Start Pose of our robot.
    private final Pose ReadMotifPose = new Pose(92, 86, Math.toRadians(270));
    private final Pose scorePose = new Pose(94, 88, Math.toRadians(227)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    private final Pose pickup1SetUpPose = new Pose(112, 87, Math.toRadians(0));
    private final Pose pickup1EndPose = new Pose(135, 87, Math.toRadians(0));
    private final Pose pickup2SetUpPose = new Pose(112, 60, Math.toRadians(0));
    private final Pose pickup2EndPose = new Pose(140, 60, Math.toRadians(0));
    private final Pose notOpenGatePose = new Pose(115, 60, Math.toRadians(0));
    private final Pose endingPose = new Pose(125, 70, Math.toRadians(270)); // Middle (Second Set) of Artifacts from the Spike Mark.
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 4:
                ...
                if(!follower.isBusy() && pathTimer.getElapsedTime() >= 1800) {
                    intake.setPower(1);
    ...
    public void ShootAllBalls(int nextPath, PathChain nextPathChain, int ballIndexId) {
        ...
        }  else if (((pathTimer.getElapsedTime() > 1000 && pathTimer.getElapsedTime() < 1500) || (pathTimer.getElapsedTime() > 1000 + secondShot && pathTimer.getElapsedTime() < 1500 + secondShot)) && nextSlot) {
            currentslot = RotateMotorToNextSlotEncoder(sorter, currentslot, false);
            nextSlot = false;
            UpdateLEDs(currentArtifacts, currentslot, prismDriver, slot0, slot1, slot2, slot3, slot4, slot5, slot6, slot7, slot8);
        } else if ((pathTimer.getElapsedTime() > 750 && pathTimer.getElapsedTime() < 1000) || (pathTimer.getElapsedTime() > 750 + secondShot && pathTimer.getElapsedTime() < 1000 + secondShot) || (pathTimer.getElapsedTime() > 750 + thirdShot && pathTimer.getElapsedTime() < 1000 + thirdShot)) {
            outtakeFeeder.setPosition(0.05);
        } else if ((pathTimer.getElapsedTime() > 500 && pathTimer.getElapsedTime() < 750) || (pathTimer.getElapsedTime() > 500 + secondShot && pathTimer.getElapsedTime() < 750 + secondShot) || (pathTimer.getElapsedTime() > 500 + thirdShot && pathTimer.getElapsedTime() < 750 + thirdShot)) {
